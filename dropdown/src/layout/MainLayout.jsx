    ...
    return (
        <div className="h-screen flex flex-col">
            <header className="text-white bg-[#313030] relative flex justify-between">
                <nav className="flex">
                    <button onClick={() => navigate('/') } className="px-3 cursor-pointer">
                    ...
                    <div className="relative">
                        <p onClick={() => {
                                ...
                                dispatch({type:"closeSubNav"})
                            }} className={` ${ state.topnav ? "bg-red-600" : "" } py-3 px-8 cursor-pointer hover:bg-red-600 flex items-center gap-1`}>topnav 
                            <i className="bi bi-caret-down-fill translate-px "></i>
                        </p>
                        <div className={`${ state.topnav ? "h-60" : "h-0"} w-full duration-200 ease-in transition-all overflow-hidden flex flex-col bg-gray-600 absolute absolute z-10`}>
                         { paths.map( path => 
                ...
                <button onClick={ () => {
                                ...
                                dispatch({type:"closeTopNav"})
                                } } className={` ${ state.subnav ? "bg-red-600" : "" } py-3 px-5 cursor-pointer hover:bg-red-600 flex gap-1 items-center`}>
                        subnav
                        <i className="bi bi-caret-down-fill translate-px"></i>
                </button>
                </nav>
                ...
                { state.route ?  <button onClick={ () => { dispatch({type:"toggleSideBar"}) 
                                          dispatch({type:"closeSubNav"})   
                                          dispatch({type:"closeTopNav"}) 
                } } className="cursor-pointer px-4 bg-black ease-linear duration-75 hover:bg-[#5b5a5a]">
                        <i className="bi bi-list text-2xl"></i>
                </button>
                : "" }

                <button className="cursor-pointer px-4 bg-black ease-linear duration-75 hover:bg-[#5b5a5a]">
                        <i className="bi bi-list text-2xl"></i>
                </button>
